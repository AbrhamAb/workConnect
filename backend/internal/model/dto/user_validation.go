...
func (r RegisterRequest) Validate() error {
	return validation.ValidateStruct(&r,
		...
		validation.Field(&r.ProfileImage,
			validation.Length(0, 8*1024*1024),
		),
		validation.Field(&r.ProfilePicture,
			validation.Length(0, 8*1024*1024),
		),
	)
}
...
func (r ChangePasswordRequest) Validate() error {
	return validation.ValidateStruct(&r,
		validation.Field(&r.CurrentPassword, validation.Required.Error("current password is required")),
		validation.Field(&r.NewPassword,
			validation.Required.Error("new password is required"),
			validation.Length(8, 100).Error("new password must be at least 8 characters"),
		),
	)
}

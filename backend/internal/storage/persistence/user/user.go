...
func (s *sqlStore) UpdatePasswordHash(ctx context.Context, userID int64, passwordHash string) error {
...
func (s *sqlStore) DeactivateUser(ctx context.Context, userID int64) error {
	q := `
		UPDATE users
...
func (s *sqlStore) GetWorkerDetails(ctx context.Context, workerID int64) (db.WorkerDetails, error) {
	q := `
		SELECT
			...
			(
				...
				WHERE active_requests.worker_id = wp.id
				  AND active_requests.status IN ('accepted', 'in_progress')
			) AS active_jobs,
			COALESCE(sc.name, '') AS category_name,
			wp.bio,
			u.phone,
	...
	err := s.db.QueryRowContext(ctx, q, workerID).Scan(
		...
		&details.Worker.ExperienceYears,
		&details.Worker.ResponseRate,
		&details.Worker.ActiveJobs,
...
func (s *sqlStore) ListAllServiceRequests(ctx context.Context) ([]db.ServiceRequestView, error) {
	q := `
		...
		ORDER BY sr.created_at DESC
	`
	return s.scanServiceRequests(ctx, q)
}
...
func (s *sqlStore) scanServiceRequests(ctx context.Context, q string, args ...any) ([]db.ServiceRequestView, error) {

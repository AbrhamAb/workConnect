...
func (h *Handler) HealthCheck(w nethttp.ResponseWriter, r *nethttp.Request) {
	response.SendSuccessResponse(w, r, nethttp.StatusOK, "🚀 workConnect service is running", nil, false)
}
...
func (h *Handler) ChangePassword(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if err = h.Module().WorkConnect.ChangePassword(r.Context(), principal.UserID, req); err != nil {
		response.SendErrorResponse(w, r, err)
		return
	}

	response.SendSuccessResponse(w, r, nethttp.StatusOK, "password updated", nil)
}
...
func (h *Handler) DeleteAccount(w nethttp.ResponseWriter, r *nethttp.Request) {
	principal, err := h.requirePrincipal(r.Context())
	if err != nil {
		response.SendErrorResponse(w, r, err)
		return
	...
	if err = h.Module().WorkConnect.DeleteAccount(r.Context(), principal.UserID); err != nil {
		response.SendErrorResponse(w, r, err)
		return
	}

	response.SendSuccessResponse(w, r, nethttp.StatusOK, "account deleted", nil)
}
...
func (h *Handler) AdminUsers(w nethttp.ResponseWriter, r *nethttp.Request) {
	users, err := h.Module().WorkConnect.ListUsers(r.Context())
	if err != nil {
		response.SendErrorResponse(w, r, err)
		return
	}
	response.SendSuccessResponse(w, r, nethttp.StatusOK, "users fetched", map[string]any{"users": users})
}

func (h *Handler) AdminRequests(w nethttp.ResponseWriter, r *nethttp.Request) {
	requests, err := h.Module().WorkConnect.ListAllServiceRequests(r.Context())
	if err != nil {
		response.SendErrorResponse(w, r, err)
		return
	}
	response.SendSuccessResponse(w, r, nethttp.StatusOK, "requests fetched", map[string]any{"requests": requests})
}
...
func (h *Handler) requirePrincipal(ctx context.Context) (user.AuthPrincipal, error) {

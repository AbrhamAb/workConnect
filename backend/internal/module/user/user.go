...
func (m *WorkConnectModule) Register(ctx context.Context, req dto.RegisterRequest) (string, db.User, error) {
	...
	if profileImage == "" {
		profileImage = strings.TrimSpace(req.ProfilePicture)
	}
	...
	if profileImage != "" {
		if _, err = m.store.UpdateProfileImage(ctx, user.ID, profileImage); err != nil {
			return "", db.User{}, err
		}
	}
...
func ensureUserIsActive(user db.User) error {
	if !user.IsActive {
...
func (m *WorkConnectModule) Login(ctx context.Context, req dto.LoginRequest) (*dto.UserLoginResponse, error) { // login response should be pointer to avoid unnecessary copying and i have changed dto response type
	...
	if err = ensureUserIsActive(user); err != nil {
		return nil, err
	}
...
func (m *WorkConnectModule) ChangePassword(ctx context.Context, userID int64, req dto.ChangePasswordRequest) error {
	if err := req.Validate(); err != nil {
		return err
	}
...
func (m *WorkConnectModule) DeleteAccount(ctx context.Context, userID int64) error {
	...
	if err := m.store.DeactivateUser(ctx, userID); err != nil {
		return err
	}
...
func (m *WorkConnectModule) ListAllServiceRequests(ctx context.Context) ([]db.ServiceRequestView, error) {
	return m.store.ListAllServiceRequests(ctx)
}

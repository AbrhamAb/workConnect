...
import (
	...
	admin "task-management-backend/internal/module/admin"
	auth "task-management-backend/internal/module/auth"
	customer "task-management-backend/internal/module/customer"
	shared "task-management-backend/internal/module/shared"
	user "task-management-backend/internal/module/user"
	worker "task-management-backend/internal/module/worker"
...
type WorkConnectService interface {
	...
	ListWorkers(ctx context.Context, category, city, q, sort string) ([]db.WorkerCard, error)
...
// Module aggregates all role-based services
type Module struct {
...
func New(db *sql.DB) *Module {
	...
	return &Module{
		WorkConnect: &workConnect{
			AuthModule:     auth.NewAuthModule(store, jwtSecret),
			SharedModule:   shared.NewSharedModule(store, jwtSecret),
			CustomerModule: customer.NewCustomerModule(store),
			WorkerModule:   worker.NewWorkerModule(store),
			AdminModule:    admin.NewAdminModule(store),
		},
	}
}
